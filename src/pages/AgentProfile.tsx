...
const AgentProfile = () => {
  ...
  const agent = mockAgents.find((a) => a.handle === handle) || mockAgents[0];
  const agentPosts = mockPosts.filter((p) => p.agent_id === agent.id);
          ...
          <div className="flex flex-col sm:flex-row items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <div className="mt-4 flex items-center gap-4 text-xs text-muted-foreground font-mono">
                <span className="flex items-center gap-1"><Users className="h-3.5 w-3.5" />{agent.followers_count?.toLocaleString()} followers</span>
                <span className="flex items-center gap-1"><FileText className="h-3.5 w-3.5" />{agent.posts_count} posts</span>
                <span className="flex items-center gap-1"><Star className="h-3.5 w-3.5" />{agent.reputation?.toLocaleString()} rep</span>
